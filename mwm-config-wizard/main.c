 ...
 *
 * mwm-config-wizard: Program to convert configs using keycodes to configs using
 *                   keysyms.
 *
 */
#include <config.h>
...
#include "mwm-config-wizard-atoms.xmacro.h"
...
/*
 * Having verboselog(), errorlog() and debuglog() is necessary when using libmwm.
 *
 */
...
static int handle_expose(void) {
    ...
    if (current_step == STEP_WELCOME) {
        txt(2, 2, "You have not configured mwm yet.", white, black);
...
/*
 * Creates the config file and tells mwm to reload.
 *
 */
static void finish(void) {
    printf("creating \"%s\"...\n", config_path);
    ...
    FILE *kc_config = fopen(SYSCONFDIR "/mwm/config.keycodes", "r");
    if (kc_config == NULL) {
        err(1, "Could not open input file \"%s\"", SYSCONFDIR "/mwm/config.keycodes");
    }
    ...
    /* tell mwm to reload the config file */
...
int main(int argc, char *argv[]) {
    ...
    /* Always write to $XDG_CONFIG_HOME/mwm/config by default. */
    if ((xdg_config_home = getenv("XDG_CONFIG_HOME")) == NULL) {
        xdg_config_home = "~/.config";
    }

    xdg_config_home = resolve_tilde(xdg_config_home);
    sasprintf(&config_path, "%s/mwm/config", xdg_config_home);

    /* Create $XDG_CONFIG_HOME/mwm if it does not yet exist */
    char *config_dir;
    struct stat stbuf;
    sasprintf(&config_dir, "%s/mwm", xdg_config_home);
...
#undef xmacro
    ...
    set_font(&font);
    char_width = predict_text_width(mwmstring_from_utf8("a"));
...
#undef xmacro
    ...
    xcb_change_property(conn,
                        ...
                        A_UTF8_STRING,
                        8,
                        strlen("mwm: first configuration"),
                        "mwm: first configuration");
