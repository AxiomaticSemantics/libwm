...
char *get_config_path(const char *override_configpath, bool use_system_paths) {
    ...
    /* 1: check for $XDG_CONFIG_HOME/mwm/config */
    ...
    config_path = resolve_tilde("~/.mwm/config");
    if (path_exists(config_path)) {
        return config_path;
    }
    free(config_path);
    ...
    /* 3: check for $XDG_CONFIG_DIRS/mwm/config */
    ...
    while (tok != NULL) {
        ...
        sasprintf(&config_path, "%s/mwm/config", tok);
        free(tok);
        if (path_exists(config_path)) {
            free(buf);
            return config_path;
    ...
    config_path = SYSCONFDIR "/mwm/config";
    if (path_exists(config_path)) {
