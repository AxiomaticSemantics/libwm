 ...
 *
 * mwm - an i3 derived improved tiling window manager
...
int ipc_recv_message(int sockfd, uint32_t *message_type,
    ...
    const uint32_t to_read = strlen(MWM_IPC_MAGIC) + sizeof(uint32_t) + sizeof(uint32_t);
    char msg[to_read];
    char *walk = msg;
    ...
    if (memcmp(walk, MWM_IPC_MAGIC, strlen(MWM_IPC_MAGIC)) != 0) {
        ELOG("IPC: invalid magic in header, got \"%.*s\", want \"%s\"\n",
             (int)strlen(MWM_IPC_MAGIC), walk, MWM_IPC_MAGIC);
        return -3;
    }
