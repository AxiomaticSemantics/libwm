...
static void check_crossing_screen_boundary(uint32_t x, uint32_t y) {
    ...
    if (output->con == NULL) {
        ELOG("ERROR: The screen is not recognized by mwm (no container associated)\n");
        return;
    }
...
#define COPY_MASK_MEMBER(mask_member, event_member) \
    ...
    if (workspace && (strcmp(workspace->name, "__mwm_scratch") == 0)) {
...
static void handle_screen_change(xcb_generic_event_t *e) {
    ...
    ipc_send_event("output", MWM_IPC_EVENT_OUTPUT, "{\"change\":\"unspecified\"}");
}
...
ignore_end:
    /* If the client (as opposed to mwm) destroyed or unmapped a window, an
     * EnterNotify event will follow (indistinguishable from an EnterNotify
     * event caused by moving your mouse), causing mwm to set focus to whichever
...
static bool window_name_changed(mwmWindow *window, char *old_name) {
    ...
    return (strcmp(old_name, mwmstring_as_utf8(window->name)) != 0);
}
...
static bool handle_windowname_change(Con *con, xcb_get_property_reply_t *prop) {
    char *old_name = (con->window->name != NULL ? sstrdup(mwmstring_as_utf8(con->window->name)) : NULL);
...
static void handle_client_message(xcb_client_message_event_t *event) {
    ...
    if (event->type == A__NET_WM_STATE) {
        ...
        if (con_is_internal(ws) && ws != workspace_get("__mwm_scratch")) {
        ...
        if (event->data.data32[0] == 2) {
            ...
            if (con_is_internal(ws)) {
                ...
                workspace_show(ws);
                /* Re-set focus, even if unchanged from mwm’s perspective. */
                focused_id = XCB_NONE;
                con_activate_unblock(con);
            }
        } else {
...
/*
 * Handles FocusIn events which are generated by clients (mwm’s focus changes
...
static void handle_focus_in(xcb_focus_in_event_t *event) {
    ...
    /* Skip dock clients, they cannot get the mwm focus. */
...
/*
 * Handles the _MWM_FLOATING_WINDOW property to properly run assignments for
 ...
 *
 */
static bool handle_mwm_floating(Con *con, xcb_get_property_reply_t *prop) {
...
static struct property_handler_t property_handlers[] = {
    ...
    {0, UINT_MAX, handle_mwm_floating},
...
void property_handlers_init(void) {
    ...
    property_handlers[10].atom = A_MWM_FLOATING_WINDOW;
