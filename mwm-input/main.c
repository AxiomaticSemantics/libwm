 ...
 *
 * mwm-input/main.c: Utility which lets the user input commands and sends them
 *                  to mwm.
 *
 */
#include <getopt.h>
...
/* Exit codes for mwm-input:
 * 0 if mwm-input exited successfully and the command was run
 * 1 if the user canceled input
 * 2 if mwm-input fails for any other reason */
const int EXIT_OK = 0;
const int EXIT_CANCEL = 1;
const int EXIT_ERROR = 2;

/* IPC format string. %s will be replaced with what the user entered, then
 * the command will be sent to mwm */
static char *format;
 ...
 * rendering it (UCS-2) or sending it to mwm (UTF-8).
 *
 */
...
static int handle_key_press(void *ignored, xcb_connection_t *conn, xcb_key_press_event_t *event) {
    ...
    // TODO: port the input handling code from mwm-lock once libxkbcommon ≥ 0.5.0
...
static xcb_rectangle_t get_window_position(void) {
    ...
    /* We need to find out whether the input focus is on an mwm frame window. If it is, we must not translate the coordinates. */
    if (wm_class == NULL || xcb_get_property_value_length(wm_class) == 0 || strcmp(xcb_get_property_value(wm_class), "mwm-frame") != 0) {
