 ...
 *
 * mwm-nagbar is a utility which displays a nag message, for example in the case
...
#include "mwm-nagbar-atoms.xmacro.h"
...
static void handle_button_release(xcb_connection_t *conn, xcb_button_release_event_t *event) {
     ...
     * mwm-sensible-terminal supports -e with multiple arguments (and not
    ...
    char *terminal_cmd;
    if (button->terminal) {
        sasprintf(&terminal_cmd, "mwm-sensible-terminal -e %s", link_path);
    } else {
...
int main(int argc, char *argv[]) {
     ...
     * _MWM_NAGBAR_CMD. But turns out that some terminal emulators such as
     ...
     *
     * Therefore, we symlink mwm-nagbar (which MUST reside on an executable
     * filesystem) with a special name and run that symlink. When mwm-nagbar
     ...
     *
     * From a security point of view, mwm-nagbar is just an alias to /bin/sh in
    ...
    prompt = mwmstring_from_utf8("Please do not run this program.");
    ...
    btn_close.label = mwmstring_from_utf8("X");
