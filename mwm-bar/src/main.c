...
static void print_usage(char *elf_name) {
    ...
    printf("\n");
    printf("-b, --bar_id       <bar_id>\tBar ID for which to get the configuration, defaults to the first bar from the mwm config\n");
    printf("-s, --socket       <sock_path>\tConnect to mwm via <sock_path>\n");
    printf("-t, --transparency Enable transparency (RGBA colors)\n");
    printf("-h, --help         Display this help message and exit\n");
    printf("-v, --version      Display version number and exit\n");
    printf("-V, --verbose      Enable verbose mode\n");
    printf("\n");
    printf(" PLEASE NOTE that mwm-bar will be automatically started by mwm\n"
...
int main(int argc, char **argv) {
    ...
    while ((opt = getopt_long(argc, argv, "b:s:thvV", long_opt, &option_index)) != -1) {
        ...
        case 's':
            ...
            break;
        case 'v':
            printf("mwm-bar version " MWM_VERSION " © 2010 Axel Wagner and contributors\n");
            exit(EXIT_SUCCESS);
            break;
        case 'b':
    ...
    LOG("mwm-bar version " MWM_VERSION "\n");
    ...
    if (socket_path == NULL) {
        char *mwm_default_sock_path = "/tmp/mwm-ipc.sock";
        ELOG("No socket path specified, default to %s\n", mwm_default_sock_path);
        socket_path = sstrdup(mwm_default_sock_path);
    }
