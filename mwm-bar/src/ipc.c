...
/*
 * Returns true when mwm-bar is configured to read workspace information from mwm
 * via JSON over the mwm IPC interface, as opposed to reading workspace
 * information from the workspace_command via JSON over stdout.
 *
 */
static bool mwm_provides_workspaces(void) {
    return !config.disable_ws && config.workspace_command == NULL;
}

/*
 * Called, when we get a reply to a command from mwm.
 * Since mwm does not give us much feedback on commands, we do not much
 *
 */
...
static void got_output_reply(const unsigned char *reply, size_t size) {
    ...
    if (mwm_provides_workspaces()) {
        mwm_send_msg(MWM_IPC_MESSAGE_TYPE_GET_WORKSPACES, NULL);
         ...
         * and re-send the output information to mwm-bar. Even in that case
         * though there is a race condition where the child can send the new
         * workspace information after the output change before mwm-bar receives
         * the output event from mwm. For this reason, we re-parse the latest
         * received JSON. */
        repeat_last_ws_json();
    }
...
static void got_bar_config(const unsigned char *reply, size_t size) {
    if (!config.bar_id) {
        ...
        parse_get_first_mwm_bar_config(reply, size);

        if (!config.bar_id) {
            ELOG("No bar configuration found, please configure a bar block in your mwm config file.\n");
            exit(EXIT_FAILURE);
        }

        LOG("Using first bar config: %s. Use --bar_id to manually select a different bar configuration.\n", config.bar_id);
        mwm_send_msg(MWM_IPC_MESSAGE_TYPE_GET_BAR_CONFIG, config.bar_id);
        return;
    }
    ...
    mwm_send_msg(MWM_IPC_MESSAGE_TYPE_GET_OUTPUTS, NULL);
...
handler_t reply_handlers[] = {
    &got_command_reply,   /* MWM_IPC_REPLY_TYPE_COMMAND */
    &got_workspace_reply, /* MWM_IPC_REPLY_TYPE_WORKSPACES */
    &got_subscribe_reply, /* MWM_IPC_REPLY_TYPE_SUBSCRIBE */
    &got_output_reply,    /* MWM_IPC_REPLY_TYPE_OUTPUTS */
    NULL,                 /* MWM_IPC_REPLY_TYPE_TREE */
    NULL,                 /* MWM_IPC_REPLY_TYPE_MARKS */
    &got_bar_config,      /* MWM_IPC_REPLY_TYPE_BAR_CONFIG */
    NULL,                 /* MWM_IPC_REPLY_TYPE_VERSION */
    NULL,                 /* MWM_IPC_REPLY_TYPE_BINDING_MODES */
    NULL,                 /* MWM_IPC_REPLY_TYPE_CONFIG */
    NULL,                 /* MWM_IPC_REPLY_TYPE_TICK */
    NULL,                 /* MWM_IPC_REPLY_TYPE_SYNC */
};
...
/*
 * Called, when a mode event arrives (mwm changed binding mode).
 *
 */
...
/*
 * Called, when a barconfig_update event arrives (i.e. mwm changed the bar hidden_state or mode)
 *
 */
...
/*
 * Called, when we get a message from mwm
 *
 */
static void got_data(struct ev_loop *loop, ev_io *watcher, int events) {
    ...
    uint32_t header_len = strlen(MWM_IPC_MAGIC) + sizeof(uint32_t) * 2;
    ...
    while (rec < header_len) {
        ...
        if (n == 0) {
            /* EOF received. Since mwm will restart mwm-bar instances as appropriate,
             * we exit here. */
    ...
    if (strncmp(header, MWM_IPC_MAGIC, strlen(MWM_IPC_MAGIC))) {
        ELOG("Wrong magic code: %.*s\n Expected: %s\n",
             (int)strlen(MWM_IPC_MAGIC),
             header,
             MWM_IPC_MAGIC);
        exit(EXIT_FAILURE);
    }

    char *walk = header + strlen(MWM_IPC_MAGIC);
    uint32_t size;
...
int mwm_send_msg(uint32_t type, const char *payload) {
    ...
    uint32_t to_write = strlen(MWM_IPC_MAGIC) + sizeof(uint32_t) * 2 + len;
    ...
    memcpy(buffer, MWM_IPC_MAGIC, strlen(MWM_IPC_MAGIC));
    walk += strlen(MWM_IPC_MAGIC);
    ...
    swrite(mwm_connection->fd, buffer, to_write);
...
void init_connection(const char *socket_path) {
    ...
    mwm_connection = smalloc(sizeof(ev_io));
    ev_io_init(mwm_connection, &got_data, sockfd, EV_READ);
    ev_io_start(main_loop, mwm_connection);
}
...
void destroy_connection(void) {
    close(mwm_connection->fd);
    ev_io_stop(main_loop, mwm_connection);
}

/*
 * Subscribe to all the mwm-events, we need
 *
 */
void subscribe_events(void) {
    if (mwm_provides_workspaces()) {
        mwm_send_msg(MWM_IPC_MESSAGE_TYPE_SUBSCRIBE, "[ \"workspace\", \"output\", \"mode\", \"barconfig_update\" ]");
    } else {
        mwm_send_msg(MWM_IPC_MESSAGE_TYPE_SUBSCRIBE, "[ \"output\", \"mode\", \"barconfig_update\" ]");
    }
}
