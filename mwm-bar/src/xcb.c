...
/* This is needed for integration with libmwm */
...
static mwm_output *output_for_tray;
 ...
 *
 */
static void draw_separator(mwm_output *output, uint32_t x, struct status_block *block, bool use_focus_colors) {
...
static void predict_block_length(struct status_block *block) {
    mwmString *text = block->full_text;
    ...
    if (mwmstring_get_num_bytes(text) == 0) {
 ...
 */
static void draw_statusline(mwm_output *output, uint32_t clip_left, bool use_focus_colors) {
    struct status_block *block;
...
static bool execute_custom_command(xcb_keycode_t input_code, bool event_is_release) {
    ...
    TAILQ_FOREACH (binding, &(config.bindings), bindings) {
        ...
        mwm_send_msg(MWM_IPC_MESSAGE_TYPE_RUN_COMMAND, binding->command);
        return true;
    }
    return false;
}

static void child_handle_button(xcb_button_press_event_t *event, mwm_output *output, uint32_t statusline_x, uint32_t statusline_y) {
...
static void focus_workspace(mwm_ws *ws) {
    char *buffer = NULL;
    if (ws->id != 0) {
    ...
    mwm_send_msg(MWM_IPC_MESSAGE_TYPE_RUN_COMMAND, buffer);
    free(buffer);
}
...
static void handle_button(xcb_button_press_event_t *event) {
    ...
    mwm_ws *cur_ws = NULL, *clicked_ws = NULL, *ws_walk;
...
static trayclient *trayclient_and_output_from_window(xcb_window_t win, mwm_output **output) {
    mwm_output *o_walk;
...
static void handle_client_message(xcb_client_message_event_t *event) {
    if (event->type == atoms[MWM_SYNC]) {
        xcb_window_t window = event->data.data32[0];
        uint32_t rnd = event->data.data32[1];
        /* Forward the request to mwm via the IPC interface so that all pending
         * IPC messages are guaranteed to be handled. */
        char *payload = NULL;
        sasprintf(&payload, "{\"rnd\":%d, \"window\":%d}", rnd, window);
        mwm_send_msg(MWM_IPC_MESSAGE_TYPE_SYNC, payload);
        free(payload);
        ...
        if (op == SYSTEM_TRAY_REQUEST_DOCK) {
             ...
             * killed or normal exit does not matter) of mwm-bar, these clients
             * will be correctly reparented to their most closest living
             * ancestor. Without this, tray icons might die when mwm-bar
             * exits/crashes. */
...
#include "xcb_atoms.def"
    ...
    char *path = root_atom_contents("MWM_SOCKET_PATH", xcb_connection, screen);
 ...
 *
 */
void kick_tray_clients(mwm_output *output) {
 ...
 *
 */
void destroy_window(mwm_output *output) {
...
/* Strut partial tells mwm where to reserve space for mwm-bar. This is determined
 * by the `position` bar config directive. */
static xcb_void_cookie_t config_strut_partial(mwm_output *output) {
 ...
 *   primary.
 */
static mwm_output *get_tray_output(void) {
    mwm_output *output = NULL;
...
void reconfig_windows(bool redraw_bars) {
    ...
    SLIST_FOREACH (walk, outputs, slist) {
        ...
        if (walk->bar.id == XCB_NONE) {
            ...
            /* If hide_on_modifier is set to hide or invisible mode, mwm is not supposed to manage our bar windows */
            ...
            char *class;
            int len = sasprintf(&class, "%s%cmwm-bar%c", config.bar_id, 0, 0);
            ...
            char *name;
            sasprintf(&name, "mwm-bar for output %s", walk->name);
            ...
            /* We want dock windows (for now). When override_redirect is set, mwm is ignoring
             * this one */
...
static void draw_button(surface_t *surface, color_t fg_color, color_t bg_color, color_t border_color,
                        int x, int width, int text_width, mwmString *text) {
        ...
        if (!config.disable_ws) {
            ...
            TAILQ_FOREACH (ws_walk, outputs_walk->workspaces, tailq) {
                DLOG("Drawing button for WS %s at x = %d, len = %d\n",
                     mwmstring_as_utf8(ws_walk->name), workspace_width, ws_walk->name_width);
                ...
                if (ws_walk->urgent) {
                    DLOG("WS %s is urgent!\n", mwmstring_as_utf8(ws_walk->name));
                    fg_color = colors.urgent_ws_fg;
                    bg_color = colors.urgent_ws_bg;
                    border_color = colors.urgent_ws_border;
                    unhide = true;
