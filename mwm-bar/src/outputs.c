...
static int outputs_start_map_cb(void *params_) {
    struct outputs_json_params *params = (struct outputs_json_params *)params_;
    mwm_output *new_output = NULL;

    if (params->cur_key == NULL) {
        new_output = smalloc(sizeof(mwm_output));
...
static void clear_output(mwm_output *output) {
    FREE(output->name);
...
static int outputs_end_map_cb(void *params_) {
    ...
    mwm_output *target = get_output_by_name(params->outputs_walk->name);
