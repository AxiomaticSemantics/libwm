...
static int mode_end_map_cb(void *params_) {
    ...
    /* Save the name */
    params->mode->name = mwmstring_from_utf8(params->name);
    mwmstring_set_markup(params->mode->name, params->pango_markup);
    /* Save its rendered width */
    params->mode->name_width = predict_text_width(params->mode->name);

    DLOG("Got mode change: %s\n", mwmstring_as_utf8(params->mode->name));
    FREE(params->cur_key);
...
void parse_mode_json(const unsigned char *json, size_t size) {
    ...
    if (strcmp("default", mwmstring_as_utf8(params.mode->name)) == 0) {
        MWMSTRING_FREE(params.mode->name);
    }
