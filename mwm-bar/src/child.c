...
mwm_bar_child status_child = {0};
mwm_bar_child ws_child = {0};
...
void clear_statusline(struct statusline_head *head, bool free_resources) {
    ...
    while (!TAILQ_EMPTY(head)) {
        ...
        if (free_resources) {
            MWMSTRING_FREE(first->full_text);
            MWMSTRING_FREE(first->short_text);
            FREE(first->color);
            FREE(first->name);
            FREE(first->instance);
...
__attribute__((format(printf, 1, 2))) static void set_statusline_error(const char *format, ...) {
    ...
    err_block->full_text = mwmstring_from_utf8("Error: ");
    ...
    message_block->full_text = mwmstring_from_utf8(message);
 ...
 *
 */
static void cleanup(mwm_bar_child *c) {
    DLOG_CHILD(*c);
    ...
    FREE(c->pending_line);
    memset(c, 0, sizeof(mwm_bar_child));
}
...
static int stdin_string(void *context, const unsigned char *val, size_t len) {
    ...
    if (strcasecmp(ctx->last_map_key, "full_text") == 0) {
        ctx->block.full_text = mwmstring_from_markup_with_length((const char *)val, len);
        return 1;
    }
    if (strcasecmp(ctx->last_map_key, "short_text") == 0) {
        ctx->block.short_text = mwmstring_from_markup_with_length((const char *)val, len);
        return 1;
    }
...
static int stdin_end_map(void *context) {
     ...
     * mwm-bar doesn’t crash and the user gets an annoying message. */
    if (!new_block->full_text) {
        new_block->full_text = mwmstring_from_utf8("SPEC VIOLATION: full_text is NULL!");
    }
    if (new_block->urgent) {
        ctx->has_urgent = true;
    }
    ...
    if (new_block->min_width_str) {
        mwmString *text = mwmstring_from_utf8(new_block->min_width_str);
        mwmstring_set_markup(text, new_block->pango_markup);
        new_block->min_width = (uint32_t)predict_text_width(text);
        mwmstring_free(text);
    }

    mwmstring_set_markup(new_block->full_text, new_block->pango_markup);

    new_block->use_short = false;
    if (new_block->short_text != NULL) {
        mwmstring_set_markup(new_block->short_text, new_block->pango_markup);
    }
...
static int stdin_end_array(void *context) {
    ...
    TAILQ_FOREACH (current, &statusline_head, blocks) {
        DLOG("full_text = %s\n", mwmstring_as_utf8(current->full_text));
        DLOG("short_text = %s\n", (current->short_text == NULL ? NULL : mwmstring_as_utf8(current->short_text)));
        DLOG("color = %s\n", current->color);
    }
    DLOG("end of dump\n");
    return 1;
...
static void read_flat_input(char *buffer, int length) {
    ...
    first->full_text = mwmstring_from_utf8(buffer);
}
...
static bool read_json_input(unsigned char *input, int length) {
    ...
    if (status != yajl_status_ok) {
        ...
        fprintf(stderr, "[mwm-bar] Could not parse JSON input (code = %d, message = %s): %.*s\n",
...
void set_workspace_button_error(const char *message) {
    ...
    SLIST_FOREACH (output, outputs, slist) {
        mwm_ws *fake_ws = scalloc(1, sizeof(mwm_ws));
        /* Don't set the canonical_name field to make this workspace unfocusable. */
        fake_ws->name = mwmstring_from_utf8(name);
...
static void setup_child_cb(mwm_bar_child *child) {
...
static bool is_alive(mwm_bar_child *c) {
    return c->pid > 0;
}
...
static void cont_child(mwm_bar_child *c) {
...
static void kill_and_wait(mwm_bar_child *c) {
    DLOG_CHILD(*c);
    if (!is_alive(c)) {
        return;
    }
...
static void stop_child(mwm_bar_child *c) {
