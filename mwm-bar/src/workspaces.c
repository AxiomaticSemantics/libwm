...
static int workspaces_string_cb(void *params_, const unsigned char *val, size_t len) {
    ...
    if (!strcmp(params->cur_key, "name")) {
        ...
        if ((config.strip_ws_numbers || config.strip_ws_name) && params->workspaces_walk->num >= 0) {
            ...
            if (config.strip_ws_numbers) {
                ...
                params->workspaces_walk->name = (offset < len
                                                     ? mwmstring_from_markup_with_length(ws_name + offset, len - offset)
                                                     : mwmstring_from_markup(ws_num));
            } else {
                params->workspaces_walk->name = mwmstring_from_markup(ws_num);
            }
        } else {
            /* Default case: just save the name */
            params->workspaces_walk->name = mwmstring_from_markup_with_length(ws_name, len);
        }
        ...
        DLOG("Got workspace canonical: %s, name: '%s', name_width: %d, glyphs: %zu\n",
             ...
             mwmstring_as_utf8(params->workspaces_walk->name),
             params->workspaces_walk->name_width,
             mwmstring_get_num_glyphs(params->workspaces_walk->name));
        FREE(params->cur_key);
    ...
    if (!strcmp(params->cur_key, "output")) {
        ...
        mwm_output *target = get_output_by_name(output_name);
        mwm_ws *ws = params->workspaces_walk;
        if (target != NULL) {
            ws->output = target;
...
static int workspaces_start_map_cb(void *params_) {
    ...
    if (params->cur_key == NULL) {
        mwm_ws *new_workspace = scalloc(1, sizeof(mwm_ws));
        new_workspace->num = -1;
    ...
    SLIST_FOREACH (outputs_walk, outputs, slist) {
        if (outputs_walk->workspaces != NULL && !TAILQ_EMPTY(outputs_walk->workspaces)) {
            ...
            TAILQ_FOREACH (ws_walk, outputs_walk->workspaces, tailq) {
                MWMSTRING_FREE(ws_walk->name);
                FREE(ws_walk->canonical_name);
            }
            FREE_TAILQ(outputs_walk->workspaces, mwm_ws);
        }
    }
}
