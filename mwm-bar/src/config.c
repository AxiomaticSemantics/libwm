 ...
 *
 * config.c: Parses the configuration (received from mwm).
 *
 */
#include "common.h"
...
static int config_null_cb(void *params_) {
    if (!strcmp(cur_key, "id")) {
        ...
        ELOG("No such bar config. Use 'mwm-msg -t get_bar_config' to get the available configs.\n");
        ELOG("Are you starting mwm-bar by hand? You should not:\n");
        ELOG("Configure a 'bar' block in your mwm config and mwm will launch mwm-bar automatically.\n");
        exit(EXIT_FAILURE);
    }
...
static int config_string_cb(void *params_, const unsigned char *val, size_t _len) {
    ...
    if (parsing_bindings) {
        if (strcmp(cur_key, "command") == 0) {
            ...
            if (binding == NULL) {
                ELOG("There is no binding to put the current command onto. This is a bug in mwm.\n");
                return 0;
            }

            if (binding->command != NULL) {
                ELOG("The binding for input_code = %d already has a command. This is a bug in mwm.\n", binding->input_code);
                return 0;
            }
     ...
     * users updating from that version and restarting mwm-bar before mwm. */
    ...
    if (!strcmp(cur_key, "separator_symbol")) {
        DLOG("separator = %.*s\n", len, val);
        MWMSTRING_FREE(config.separator_symbol);
        config.separator_symbol = mwmstring_from_utf8_with_length((const char *)val, len);
        return 1;
    }
    ...
    /* We keep the old single tray_output working for users who only restart mwm-bar
     * after updating. */
...
static int mwm_bar_config_string_cb(void *params_, const unsigned char *val, size_t _len) {
 ...
 *
 */
void parse_get_first_mwm_bar_config(const unsigned char *json, size_t size) {
    yajl_callbacks configs_callbacks = {
        .yajl_string = mwm_bar_config_string_cb,
    };
