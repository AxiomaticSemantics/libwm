 ...
 *
 * mwm-msg/main.c: Utility which sends messages to a running mwm-instance using
 * IPC via UNIX domain sockets.
 *
 * This (in combination with libmwm/ipc_send_message.c and
 * libmwm/ipc_recv_message.c) serves as an example for how to send your own
 * messages to mwm.
 *
...
/*
 * Having verboselog() and errorlog() is necessary when using libmwm.
 *
 */
...
/* TODO: avoid needless sstrndup on invalid value */
...
int main(int argc, char *argv[]) {
    ...
    uint32_t message_type = MWM_IPC_MESSAGE_TYPE_RUN_COMMAND;
    char *payload = NULL;
    bool quiet = false;
    bool monitor = false;
    ...
    while ((o = getopt_long(argc, argv, options_string, long_options, &option_index)) != -1) {
        if (o == 's') {
            ...
            if (strcasecmp(optarg, "command") == 0) {
                message_type = MWM_IPC_MESSAGE_TYPE_RUN_COMMAND;
            } else if (strcasecmp(optarg, "run_command") == 0) {
                message_type = MWM_IPC_MESSAGE_TYPE_RUN_COMMAND;
            } else if (strcasecmp(optarg, "get_workspaces") == 0) {
                message_type = MWM_IPC_MESSAGE_TYPE_GET_WORKSPACES;
            } else if (strcasecmp(optarg, "get_outputs") == 0) {
                message_type = MWM_IPC_MESSAGE_TYPE_GET_OUTPUTS;
            } else if (strcasecmp(optarg, "get_tree") == 0) {
                message_type = MWM_IPC_MESSAGE_TYPE_GET_TREE;
            } else if (strcasecmp(optarg, "get_marks") == 0) {
                message_type = MWM_IPC_MESSAGE_TYPE_GET_MARKS;
            } else if (strcasecmp(optarg, "get_bar_config") == 0) {
                message_type = MWM_IPC_MESSAGE_TYPE_GET_BAR_CONFIG;
            } else if (strcasecmp(optarg, "get_binding_modes") == 0) {
                message_type = MWM_IPC_MESSAGE_TYPE_GET_BINDING_MODES;
            } else if (strcasecmp(optarg, "get_binding_state") == 0) {
                message_type = MWM_IPC_MESSAGE_TYPE_GET_BINDING_STATE;
            } else if (strcasecmp(optarg, "get_version") == 0) {
                message_type = MWM_IPC_MESSAGE_TYPE_GET_VERSION;
            } else if (strcasecmp(optarg, "get_config") == 0) {
                message_type = MWM_IPC_MESSAGE_TYPE_GET_CONFIG;
            } else if (strcasecmp(optarg, "send_tick") == 0) {
                message_type = MWM_IPC_MESSAGE_TYPE_SEND_TICK;
            } else if (strcasecmp(optarg, "subscribe") == 0) {
                message_type = MWM_IPC_MESSAGE_TYPE_SUBSCRIBE;
            } else {
            ...
            printf("mwm-msg " MWM_VERSION "\n");
            return 0;
        } else if (o == 'h') {
            printf("mwm-msg " MWM_VERSION "\n");
            printf("mwm-msg [-s <socket>] [-t <type>] [-m] <message>\n");
            return 0;
        } else if (o == '?') {
            exit(EXIT_FAILURE);
        } else if (o == 'r') {
    ...
    if (monitor && message_type != MWM_IPC_MESSAGE_TYPE_SUBSCRIBE) {
        fprintf(stderr, "The monitor option -m is used with -t SUBSCRIBE exclusively.\n");
        exit(EXIT_FAILURE);
    }

    /* Use all arguments, separated by whitespace, as payload.
     * This way, you don’t have to do mwm-msg 'mark foo', you can use
     * mwm-msg mark foo */
    while (optind < argc) {
        if (!payload) {
    ...
    if (reply_type == MWM_IPC_REPLY_TYPE_COMMAND) {
    ...
    } else if (reply_type == MWM_IPC_REPLY_TYPE_CONFIG) {
        if (raw_reply) {
            printf("%.*s\n", reply_length, reply);
        } else {
        ...
        do {
            ...
            if (!(reply_type & MWM_IPC_EVENT_MASK)) {
