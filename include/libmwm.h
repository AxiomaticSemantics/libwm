 ...
 *
 * libmwm: contains functions which are used by mwm *and* accompanying tools such
 * as mwm-msg, mwm-config-wizard, ...
 */
#pragma once
 ...
 *
 */
typedef struct _mwmString mwmString;

typedef struct Font mwmFont;
...
/* Since this file also gets included by utilities which don’t use the mwm log
 * infrastructure, we define a fallback. */
#if !defined(LOG)
void verboselog(char *fmt, ...)
    __attribute__((format(printf, 1, 2)));
#define LOG(fmt, ...) verboselog("[libmwm] " __FILE__ " " fmt, ##__VA_ARGS__)
#endif
#if !defined(ELOG)
void errorlog(char *fmt, ...)
    __attribute__((format(printf, 1, 2)));
#define ELOG(fmt, ...) errorlog("[libmwm] ERROR: " fmt, ##__VA_ARGS__)
#endif
#if !defined(DLOG)
...
/**
 * Try to get the contents of the given atom (for example MWM_SOCKET_PATH) from
...
/**
 * Build an mwmString from an UTF-8 encoded string in Pango markup with fixed
 * length.
 *
 */
...
/**
 * Build an mwmString from an UCS-2 encoded string.
 * Returns the newly-allocated mwmString.
 *
 */
...
/**
 * Securely mwmstring_free by setting the pointer to NULL
...
/**
 * Returns the number of glyphs in an mwmString.
 *
 */
...
/**
 * Formats a message (payload) of the given size and type and sends it to mwm via
 ...
 *
 */
mwmFont load_font(const char *pattern, const bool fallback);

/**
 * Defines the font to be used for the forthcoming calls.
 *
 */
void set_font(mwmFont *font);
 ...
 *
 * Text must be specified as an mwmString.
 *
 */
void draw_text(mwmString *text, xcb_drawable_t drawable, xcb_gcontext_t gc,
               cairo_surface_t *surface, int x, int y, int max_width);

/**
 * Predict the text width in pixels for the given text. Text must be
 * specified as an mwmString.
 *
 */
int predict_text_width(mwmString *text);
...
/**
 * Returns true if this version of mwm is a debug build (anything which is not a
...
/**
 * Draw the given text using libmwm.
 * This function also marks the surface dirty which is needed if other means of
 * drawing are used. This will be the case when using XCB to draw text.
 *
 */
void draw_util_text(mwmString *text, surface_t *surface, color_t fg_color, color_t bg_color, int x, int y, int max_width);

/**
 * Draw the given image using libmwm.
 */
